import { NotFound, evaluatePolicy, ApprovalRequired, enqueueFix } from "@seo/core";
 ...
 */
export const POST = handler({ permission: "fix:apply_low_risk" }, async ({ session, params, correlationId }) => {
  ...
  if (!decision.allowed) {
    ...
    const { PolicyViolation } = await import("@seo/core");
