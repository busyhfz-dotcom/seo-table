...
import type { SocialStrings } from "../social/strings";
import { kindIcon } from "../../../components/icons";

type Project = { id: string; name: string; pageCap: number; crawlRate: number };

type Kind = "WEBSITE" | "INSTAGRAM" | "TELEGRAM";
const KINDS: Kind[] = ["WEBSITE", "INSTAGRAM", "TELEGRAM"];
...
export function Wizard({
  ...
  async function createProject(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (social && handle && !HANDLE.test(handle)) {
      setBusy(null);
      return setMsg({ tone: "crit", text: ss.ob_handle_invalid });
    }
    ...
    if (!result.ok)
      return setMsg({
        tone: "crit",
        text: apiErrorMessage(locale, result.failure),
      });
    // Continue with the project just created, whichever project is the default.
    startTransition(() =>
      router.push(
        `/onboarding?project=${encodeURIComponent(result.data.project.id)}`,
      ),
    );
  }
  ...
  function skipConnect() {
    ...
    startTransition(() =>
      router.push(
        `/onboarding?project=${encodeURIComponent(project.id)}&connect=skip`,
      ),
    );
  }
  ...
  async function startScan() {
    ...
    startTransition(() =>
      router.push(`/audit?project=${encodeURIComponent(project.id)}`),
    );
  }
        ...
        <div className="body">
          ...
          >
            <fieldset className="kind-pick">
              <legend>{ss.ob_type}</legend>
              <div className="kind-grid">
                {KINDS.map((k) => (
                  <label
                    key={k}
                    className={`radio kind-opt${kind === k ? " on" : ""}`}
                  >
                    <input
                      type="radio"
                      name="kind"
                      ...
                      onChange={() => setKind(k)}
                    />
                    <span className={`kind-ic ${k.toLowerCase()}`}>
                      <Icon name={kindIcon(k)} />
                    </span>
                    <span>
                      <b>{ss[`ob_${k}`]}</b>
                      <span className="hint">{ss[`ob_${k}_desc`]}</span>
                    </span>
                  </label>
                ))}
              </div>
            ...
            <div className="grid g2" style={{ gap: 14 }}>
              ...
              {social ? (
                <label className="field">
                  <span>
                    {kind === "INSTAGRAM" ? ss.ob_handle_IG : ss.ob_handle_TG}
                  </span>
                  <input
                    id="p-handle"
                    name="handle"
                    ...
                    maxLength={33}
                  />
                  <span className="hint">{ss.ob_handle_help}</span>
                </label>
              ) : (
                <label className="field">
  ...
  switch (method.kind) {
    ...
    case "WORDPRESS_BRIDGE":
      return (
        <BridgeMethod
          method={method}
          wordpress={view.methods.find((m) => m.kind === "WORDPRESS")}
        />
      );
    default:
...
function ConnectStep({
  ...
  const applicable = view.methods.filter(
    (m) => m.status !== "not_applicable" || m.kind === "WORDPRESS",
  );
  const others = applicable.filter((m) => m !== recommended);
  const [open, setOpen] = useState<string | null>(
    recommended ? null : (others[0]?.kind ?? null),
  );
  ...
  return (
    <>
      <PlatformCard
        ...
        detectedLabel={view.detectedLabel}
        compact
      />
        ...
        <section className="card stack">
          ...
          <div className="body">
            <div
              className="seg"
              role="group"
              aria-label={recommended ? s.ob_other_methods : s.ob_step3}
            >
              {others.map((m) => (
                <button
                  key={m.kind}
            ...
            {others.map((m) =>
              open === m.kind ? (
                <MethodFor key={m.kind} method={m} view={view} />
              ) : null,
            )}
          </div>
        </section>
